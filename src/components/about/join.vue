<template>
	<div class="join_bg">
		<div class="join_img">
		    <img src="../../assets/imgs/join.png"  class = "about_lamp" v-if = "!showSmall">
		    <img src="../../assets/imgs/join_s.png" class = "about_lamp" v-if = "showSmall">
		    <h2>加入我们</h2>
		</div>
		<div class="join">
			<ul class="job_list">
				<li class = "job_item "  v-for = "(item, index) in jobList" :key = "index" v-bind:class="{ slideInLeft : (index%2) == 0 , 'slideInRight': (index%2) != 0  }">
					<h3 class = "job_name">{{item.jobName}}<span class = "job_date">发布时间：2018-08-01</span></h3>
					...
					<el-collapse-transition>
						<div class="job transition-box" v-show = "index == 0?!show : show">
							...
							<button><a href = "mailto: contact@lychee-info.cn">职位申请</a></button>
						</div>
					</el-collapse-transition>
				</li>
			</ul>
